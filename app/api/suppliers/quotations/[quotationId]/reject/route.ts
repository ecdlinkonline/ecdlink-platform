...
export async function POST(request: Request, { params }: { params: Promise<{ quotationId: string }> }) {
  ...
  try {
    ...
    return apiSuccess(await setQuotationStatus(quotation.id, "REJECTED", context.internalUser.id, body.reason));
  } catch (error) {
    if (error instanceof Error) return apiError(error.message, statusFromError(error));
    return apiError("Supplier quotation could not be rejected.", 500);
  }
}

...
import {
  listSupplierDocumentsFromDb,
  resolveSupplierDbId,
} from "@/lib/repositories/suppliers";
import { createSupplierDocument } from "@/lib/services/suppliers";
import { supplierDocumentSchema } from "@/lib/validators/suppliers";
...
export async function GET(
  ...
  return apiSuccess(
    await listSupplierDocumentsFromDb(dbId)
  );
}
...
export async function POST(
  ...
  try {
    ...
    const input = supplierDocumentSchema.parse(
      await request.json()
    );
    ...
    return apiError(
      "Supplier document could not be created.",
      500
    );
  }
}
...
export async function POST(_: Request, { params }: { params: Promise<{ insightId: string }> }) {
  ...
  try { return apiSuccess(await updateInsightStatus(context.scope, (await params).insightId, "RESOLVED")); }
  catch (error) { if (error instanceof Error) return apiError(error.message, statusFromError(error)); return apiError("Insight could not be resolved.", 500); }
}

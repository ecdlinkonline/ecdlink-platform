...
export async function POST(_: Request, { params }: { params: Promise<{ requestId: string }> }) {
  ...
  try { return apiSuccess(await setPartnershipRequestStatus((await params).requestId, "Closed", context.internalUser.id)); }
  catch (error) { if (error instanceof Error) return apiError(error.message, statusFromError(error)); return apiError("Request could not be closed.", 500); }
}

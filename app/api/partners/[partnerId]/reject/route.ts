...
export async function POST(request: Request, { params }: { params: Promise<{ partnerId: string }> }) {
  ...
  try {
    ...
    return apiSuccess(await setPartnerStatus(dbId, "Rejected", context.internalUser.id, body.reason));
  } catch (error) {
    if (error instanceof Error) return apiError(error.message, statusFromError(error));
    return apiError("Partner organisation could not be rejected.", 500);
  }
}

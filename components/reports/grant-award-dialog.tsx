...
import { grantAwardLeadOrganisation, grantAwardSourceFieldState, updateGrantAwardSource, validateGrantAwardSource, type GrantAwardFormValues } from "@/lib/grant-reports/award-form";
import { executeGrantAwardCreation } from "@/lib/grant-reports/create-award-flow";
import { restoreFileTriggerFocus } from "@/lib/grant-reports/file-input-focus";
...
export function GrantAwardDialog({ data, onSuccess }: { data: GrantReportWorkspaceData; onSuccess: () => void }) {
  ...
  async function createAward(values: GrantAwardFormValues): Promise<WorkflowActionResult> {
    try {
      return await executeGrantAwardCreation({ values, agreementFile, onPhase: setSubmissionPhase });
    } finally {
  ...
  return <WorkflowActionDialog<GrantAwardFormValues>
    ...
    confirmationButton={{ label: "Create Award", loadingLabel: submissionPhase === "uploading" ? "Uploading agreement..." : "Creating award..." }}
    fields={[]}
    ...
    renderFields={({ values, updateValue, fieldErrors, disabled }) => {
      ...
      return <>
        <Section title="1. Funding Source">
          ...
          {sourceState.showFundingApplication ? <Field label="Source Funding Application" error={fieldErrors.fundingApplicationId}><select name="fundingApplicationId" className={fieldClassName} value={values.fundingApplicationId} disabled={disabled} onChange={(event)=>updateValue("fundingApplicationId",event.target.value)}><option value="">Select an application</option>{sources.applications.map((item)=><option key={item.id} value={item.id}>{item.label}</option>)}</select></Field> : null}
          {sourceState.showSponsorshipCommitment ? <Field label="Source Sponsorship Commitment" error={fieldErrors.sponsorshipCommitmentId}><select name="sponsorshipCommitmentId" className={fieldClassName} value={values.sponsorshipCommitmentId} disabled={disabled} onChange={(event)=>updateValue("sponsorshipCommitmentId",event.target.value)}><option value="">Select a commitment</option>{sources.commitments.map((item)=><option key={item.id} value={item.id}>{item.fundingProjectId ? item.label : `${item.label} · FundingProject required`}</option>)}</select></Field> : null}
          {sourceState.relationshipFieldsDisabled ? <div className="grid gap-4 md:grid-cols-3"><ReadOnlyValue label="Centre" value={centreName}/><ReadOnlyValue label="Funding Project" value={projectName}/><ReadOnlyValue label={values.sourceType === "FUNDING_APPLICATION" ? "Lead Funder" : "Lead Donor"} value={grantAwardLeadOrganisation(values,sources) ?? ""}/></div> : <><Field label="Centre" error={fieldErrors.centreId}><select name="centreId" className={fieldClassName} value={values.centreId} disabled={disabled} onChange={(event)=>updateValue("centreId",event.target.value)}><option value="">Select a centre</option>{data.options.centres.map((item)=><option key={item.id} value={item.id}>{item.centreName}</option>)}</select></Field><Field label="Funding Project" error={fieldErrors.fundingProjectId}><select name="fundingProjectId" className={fieldClassName} value={values.fundingProjectId} disabled={disabled} onChange={(event)=>updateValue("fundingProjectId",event.target.value)}><option value="">Select a project</option>{data.options.projects.filter((item)=>!values.centreId||item.centreId===values.centreId).map((item)=><option key={item.id} value={item.id}>{item.centreName} · {item.title}</option>)}</select></Field><Field label="Funding Partner Type"><select className={fieldClassName} value={values.organisationType} disabled={disabled} onChange={(event)=>updateValue("organisationType",event.target.value)}><option value="FUNDING_ORGANISATION">Funding organisation</option><option value="DONOR_ORGANISATION">Donor organisation</option></select></Field>{values.organisationType === "FUNDING_ORGANISATION" ? <Field label="Funding Organisation" error={fieldErrors.fundingOrganisationId}><select name="fundingOrganisationId" className={fieldClassName} value={values.fundingOrganisationId} disabled={disabled} onChange={(event)=>updateValue("fundingOrganisationId",event.target.value)}><option value="">Select an organisation</option>{data.options.fundingOrganisations.map((item)=><option key={item.id} value={item.id}>{item.name}</option>)}</select></Field> : <Field label="Donor Organisation" error={fieldErrors.donorOrganisationId}><select name="donorOrganisationId" className={fieldClassName} value={values.donorOrganisationId} disabled={disabled} onChange={(event)=>updateValue("donorOrganisationId",event.target.value)}><option value="">Select an organisation</option>{data.options.donorOrganisations.map((item)=><option key={item.id} value={item.id}>{item.name}</option>)}</select></Field>}</>}
        </Section>
        <Section title="2. Award Details">
          <div className="grid gap-4 md:grid-cols-2"><Field label="Award Number" error={fieldErrors.awardNumber}><input name="awardNumber" className={fieldClassName} value={values.awardNumber} disabled={disabled} onChange={(event)=>updateValue("awardNumber",event.target.value)}/></Field><Field label="Title" error={fieldErrors.title}><input name="title" className={fieldClassName} value={values.title} disabled={disabled} onChange={(event)=>updateValue("title",event.target.value)}/></Field></div>
          ...
          <div className="grid gap-4 md:grid-cols-2"><Field label="Awarded Amount" error={fieldErrors.awardedAmount} description={amountControlled ? "Set from the authoritative source amount." : undefined}><input name="awardedAmount" className={fieldClassName} type="number" min="0" step="0.01" value={values.awardedAmount} disabled={disabled||amountControlled} onChange={(event)=>updateValue("awardedAmount",event.target.value===""?"":event.target.valueAsNumber)}/></Field><Field label="Currency"><input className={fieldClassName} maxLength={3} value={values.currency} disabled={disabled} onChange={(event)=>updateValue("currency",event.target.value)}/></Field><Field label="Start Date" error={fieldErrors.startDate}><input name="startDate" className={fieldClassName} type="date" value={values.startDate} disabled={disabled} onChange={(event)=>updateValue("startDate",event.target.value)}/></Field><Field label="End Date"><input className={fieldClassName} type="date" value={values.endDate} disabled={disabled} onChange={(event)=>updateValue("endDate",event.target.value)}/></Field></div>
        </Section>
        <Section title="3. Signed Agreement">
          <Field label="Signed Grant Agreement / Award Contract" description="Upload the agreement signed by the centre and funding partner." error={agreementError ?? fieldErrors.signedAgreementFileAssetId}><div className="relative mt-2 rounded-lg border border-dashed border-brand-line p-4 dark:border-slate-700"><input ref={agreementInputRef} id="grant-award-agreement" name="signedAgreementFileAssetId" className="pointer-events-none absolute left-0 top-0 h-px w-px opacity-0" type="file" tabIndex={-1} accept="application/pdf,.pdf" disabled={disabled} onChange={(event)=>{const file=event.target.files?.[0]??null;setAgreementFile(file);setAgreementError(file&&!(file.type==="application/pdf"&&file.name.toLowerCase().endsWith(".pdf"))?"Only PDF agreements are supported.":null);window.requestAnimationFrame(()=>restoreFileTriggerFocus(agreementTriggerRef.current));}}/><button ref={agreementTriggerRef} type="button" disabled={disabled} onClick={()=>agreementInputRef.current?.click()} className="inline-flex cursor-pointer items-center gap-2 rounded-lg bg-brand-accent px-3 py-2 text-sm font-bold text-brand-navy disabled:cursor-not-allowed disabled:opacity-60"><Upload className="h-4 w-4"/>{agreementFile?"Replace PDF":"Select PDF"}</button>{agreementFile?<p className="mt-3 flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300"><FileText className="h-4 w-4"/>{agreementFile.name}</p>:<p className="mt-3 text-xs text-slate-500">Optional · PDF · maximum 10 MB</p>}</div></Field>

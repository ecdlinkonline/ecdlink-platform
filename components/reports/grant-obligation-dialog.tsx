    ...
    renderFields={({ values, updateValue, fieldErrors, disabled }) => {
      ...
      return <>
        ...
        <Section title="2. Reporting Period">
          ...
          {state.showReportingPeriod ? <div className="grid gap-4 sm:grid-cols-2"><Field label="Reporting Period Start" error={fieldErrors.reportingPeriodStart}><input name="reportingPeriodStart" type="date" className={fieldClassName} value={values.reportingPeriodStart} disabled={disabled} readOnly={values.basis === "QUARTER"} onChange={(event) => updateValue("reportingPeriodStart", event.target.value)} /></Field><Field label="Reporting Period End" error={fieldErrors.reportingPeriodEnd}><input name="reportingPeriodEnd" type="date" className={fieldClassName} value={values.reportingPeriodEnd} disabled={disabled} readOnly={values.basis === "QUARTER"} onChange={(event) => updateValue("reportingPeriodEnd", event.target.value)} /></Field></div> : null}
